...
dest = pyproj.Proj(init='EPSG:32661')

x, y = dest(0, 60.0)
_, lat = dest(math.sqrt(2.) * y, 0., inverse=True)

minx, miny = dest(-45, lat)
maxx, maxy = dest(135, lat)
...
with Dataset("ETOPO1_Bed_g_gmt4.grd", "r") as ds_in:
    with Dataset("etopo_EPSG:32661_z%d.nc" % zoom, "w", format='NETCDF4') as ds_out:
